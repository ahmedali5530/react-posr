...
export const runNegotiationEngine = async (
  ...
  for (const supplier of suppliers) {
    ...
    if (supplier.total_spend > 5000 && supplier.order_count as any >= 10) {
      // High-volume, frequent orders → VOLUME DISCOUNT
      ruleId = 'volume_discount';
      severity = 'high';
      aiRec = 'negotiate_now';
      confidence = 0.75;
      leverage = `High volume: ${fmt$(supplier.total_spend)} over ${config.lookbackDays}d across ${supplier.order_count as any} orders — you're a top customer and have leverage to demand volume pricing.`;
      script = `"We've been spending ${fmt$(supplier.total_spend)} with you over the last 90 days. That makes us one of your top accounts. I'd like to discuss a volume discount — we're targeting ${fmt$(targetPrice)}/unit, which is ${(config.targetDiscount * 100).toFixed(0)}% off current pricing. In return, we can commit to ${supplier.order_count as any * 1.2 | 0}+ orders in the next quarter and consolidate more items with you. Can we make this work?"`;
      desc = `${supplier.supplier_name}: VOLUME DISCOUNT opportunity — ${fmt$(supplier.total_spend)} spend over ${config.lookbackDays}d, ${supplier.order_count as any} orders. Target ${fmt$(targetPrice)} (-${(config.targetDiscount * 100).toFixed(0)}%). Est savings: ${fmt$(estSavingsAnnual)}/yr.`;
    } else if (supplier.total_spend > 2000 && supplier.order_count as any >= 5) {
    ...
    } else if (supplier.order_count as any >= 8) {
      // Loyalty bonus — frequent small orders
      ruleId = 'loyalty_bonus';
      severity = 'low';
      aiRec = 'schedule_meeting';
      confidence = 0.50;
      leverage = `Loyal customer: ${supplier.order_count as any} orders in ${config.lookbackDays}d — ask for loyalty pricing or free delivery.`;
      script = `"We've placed ${supplier.order_count as any} orders with you in the last 90 days. As a loyal customer, we'd like to discuss loyalty pricing — even a 5% discount would make a big difference, or free delivery on orders over ${fmt$(supplier.total_spend / supplier.order_count as any)}."`;
      desc = `${supplier.supplier_name}: LOYALTY BONUS — ${supplier.order_count as any} orders in ${config.lookbackDays}d. Request loyalty pricing or free delivery. Est savings: ${fmt$(estSavingsAnnual)}/yr.`;
    } else {
