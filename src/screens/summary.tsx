...
export const Summary = () => {
  ...
  const taxes = useMemo(() => {
    return orders?.data?.filter(item => item.tax_amount !== undefined)?.
    reduce((prev, order) => prev + order.tax_amount, 0);
  }, [orders]);
    ...
    <Layout overflowHidden>
      <div className="flex gap-5 p-3 flex-col">
        <div className="h-[60px] flex-0 rounded-xl bg-white flex items-center px-3 gap-3">
          <div>
            <DatePicker value={date} onChange={setDate} maxValue={today(getLocalTimeZone())}/>
          </div>
        </div>
        <div className="bg-white rounded-xl w-[450px] mx-auto max-h-[calc(100vh_-_150px)] overflow-y-auto">
          {isLoading ? (
