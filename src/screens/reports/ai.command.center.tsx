...
fetchRecipeScaleSummary(db),
        ...
        fetchSubscriptionProgramSummary(db),
...
async function fetchPetFriendlySummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Pet-Friendly', faDog, 'text-amber-600', REPORTS_PET_FRIENDLY_SERVICE_ANIMAL);
    return {
      title: 'Pet-Friendly', icon: faDog, color: 'text-amber-600',
      primary: `${f.nopatio} no patio · ${f.refusals} refusals`,
      secondary: `${f.total} alerts · ${f.noprotocol} no protocol · ${f.noamenity} no amenities · ${f.nomenu} no menu · ${f.noyappy} no yappy · ${f.nophoto} no photo`,
...
async function fetchSubscriptionProgramSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Subscriptions', faRepeat, 'text-violet-600', REPORTS_SUBSCRIPTION_MEMBERSHIP_PROGRAM);
    return {
      title: 'Subscriptions', icon: faRepeat, color: 'text-violet-600',
      primary: `${f.noprogram} no program · ${f.highchurn} high churn`,
      secondary: `${f.total} alerts · ${f.subtiers} poor tiers · ${f.lowbenefit} low benefit · ${f.highfriction} high friction · ${f.nocrosssell} no cross-sell · ${f.nousage} no usage · ${f.nopricing} no pricing`,
      health: f.critical > 0 ? 'critical' : (f.noprogram > 0 || f.highchurn > 0 || f.lowbenefit > 0 ? 'warning' : 'good'), link: REPORTS_SUBSCRIPTION_MEMBERSHIP_PROGRAM, linkLabel: 'View subscriptions',
    };
  } catch { return neutralCard('Subscriptions', faRepeat, 'text-violet-600', REPORTS_SUBSCRIPTION_MEMBERSHIP_PROGRAM); }
}

async function fetchVoiceOrderingSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Voice AI', faMicrophoneLines, 'text-sky-600', REPORTS_VOICE_ORDERING_CONVERSATIONAL_AI);
    return {
      title: 'Voice AI', icon: faMicrophoneLines, color: 'text-sky-600',
      primary: `${f.nostrategy} no strategy · ${f.lowaccuracy} low accuracy`,
      secondary: `${f.total} alerts · ${f.noupsell} no upsell · ${f.nomultilingual} no multilingual · ${f.noaccent} no accent · ${f.nobiometrics} no biometrics · ${f.nohandoff} no handoff · ${f.noaccessibility} no accessibility`,
      health: f.critical > 0 ? 'critical' : (f.nostrategy > 0 || f.lowaccuracy > 0 || f.nohandoff > 0 ? 'warning' : 'good'), link: REPORTS_VOICE_ORDERING_CONVERSATIONAL_AI, linkLabel: 'View voice AI',
    };
  } catch { return neutralCard('Voice AI', faMicrophoneLines, 'text-sky-600', REPORTS_VOICE_ORDERING_CONVERSATIONAL_AI); }
}

async function fetchCrisisCommunicationSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Crisis Comms', faTriangleExclamation, 'text-rose-600', REPORTS_CRISIS_COMMUNICATION_PR_REPUTATION);
    return {
      title: 'Crisis Comms', icon: faTriangleExclamation, color: 'text-rose-600',
      primary: `${f.noplan} no plan · ${f.slowviral} slow viral`,
      secondary: `${f.total} alerts · ${f.nomonitoring} no monitoring · ${f.nofoodsafety} no food safety · ${f.nostaffscandal} no staff scandal · ${f.nomedia} no media · ${f.nolegal} no legal · ${f.norecovery} no recovery`,
      health: f.critical > 0 ? 'critical' : (f.noplan > 0 || f.slowviral > 0 || f.nofoodsafety > 0 ? 'warning' : 'good'), link: REPORTS_CRISIS_COMMUNICATION_PR_REPUTATION, linkLabel: 'View crisis comms',
    };
  } catch { return neutralCard('Crisis Comms', faTriangleExclamation, 'text-rose-600', REPORTS_CRISIS_COMMUNICATION_PR_REPUTATION); }
}

async function fetchFranchiseOperationsSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Franchise', faHandshake, 'text-violet-600', REPORTS_FRANCHISE_MULTI_UNIT_OPERATIONS);
    return {
      title: 'Franchise', icon: faHandshake, color: 'text-violet-600',
      primary: `${f.nostrategy} no strategy · ${f.lowprofit} low profit`,
      secondary: `${f.total} alerts · ${f.lowconsistency} low consistency · ${f.nosupport} no support · ${f.thinpipeline} thin pipeline · ${f.cannibalization} cannibalization · ${f.nocompliance} no compliance · ${f.weakonboarding} weak onboarding`,
...
async function fetchDataMonetizationSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Data Monet', faDatabase, 'text-violet-600', REPORTS_DATA_MONETIZATION_API_REVENUE);
    return {
      title: 'Data Monet', icon: faDatabase, color: 'text-violet-600',
      primary: `${f.nostrategy} no strategy · ${f.noapi} no API`,
      secondary: `${f.total} alerts · ${f.nolicensing} no licensing · ${f.thinpartners} thin partners · ${f.nobenchmarking} no benchmark · ${f.nopredictive} no predictive · ${f.weakprivacy} weak privacy · ${f.novaluation} no valuation`,
...
async function fetchFood3dPrintingSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('3D Food Printing', faCube, 'text-violet-600', REPORTS_3D_FOOD_PRINTING_CUSTOMIZED_CUISINE);
    return {
      title: '3D Food Printing', icon: faCube, color: 'text-violet-600',
      primary: `${f.nostrategy} no strategy · ${f.nonutrition} no nutrition`,
      secondary: `${f.total} alerts · ${f.nodietary} no dietary · ${f.nonovel} no novel · ${f.noportion} no portion · ${f.noplating} no plating · ${f.nococreation} no co-creation · ${f.noroi} no ROI`,
      health: f.critical > 0 ? 'critical' : (f.nostrategy > 0 || f.nonutrition > 0 || f.nonovel > 0 ? 'warning' : 'good'), link: REPORTS_3D_FOOD_PRINTING_CUSTOMIZED_CUISINE, linkLabel: 'View 3D printing',
    };
  } catch { return neutralCard('3D Food Printing', faCube, 'text-violet-600', REPORTS_3D_FOOD_PRINTING_CUSTOMIZED_CUISINE); }
}

async function fetchSmartRestaurantIotSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Smart IoT', faMicrochip, 'text-sky-600', REPORTS_SMART_RESTAURANT_IOT_CONNECTED_DEVICE);
    return {
      title: 'Smart IoT', icon: faMicrochip, color: 'text-sky-600',
      primary: `${f.nostrategy} no strategy · ${f.nomaintenance} no maintenance`,
      secondary: `${f.total} alerts · ${f.nofoodsafety} no food safety · ${f.nooccupancy} no occupancy · ${f.noenergy} no energy · ${f.nodashboard} no dashboard · ${f.nosecurity} no security · ${f.noroi} no ROI`,
...
async function fetchMetaverseSummary(db: any): Promise<MetricCard> {
  try {
    const result = await db.query(
      `SELECT count() AS total, math::count(severity = 'critical') AS critical,
              math::sum(est_monthly_opportunity WHERE est_monthly_opportunity > 0) AS opportunity,
    ...
    const list = Array.isArray(result) ? result.flat() : [];
    const f = list[0];
    if (!f || f.total === 0) return neutralCard('Metaverse', faGlobe, 'text-violet-600', REPORTS_METAVERSE_VIRTUAL_RESTAURANT_DINING);
    return {
      title: 'Metaverse', icon: faGlobe, color: 'text-violet-600',
      primary: `${f.nostrategy} no strategy · ${f.novirtualrestaurant} no virtual restaurant`,
      secondary: `${f.total} alerts · ${f.novrdining} no VR dining · ${f.nonftmenu} no NFT menu · ${f.nocooking} no cooking · ${f.noavatardining} no avatar · ${f.nocrossreality} no cross-reality · ${f.noevents} no events`,
